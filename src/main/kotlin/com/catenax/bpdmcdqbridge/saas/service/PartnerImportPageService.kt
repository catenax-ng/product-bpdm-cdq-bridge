...
package com.catenax.bpdmcdqbridge.saas.service

import com.catenax.bpdmcdqbridge.saas.config.SaasAdapterConfigProperties
import com.catenax.bpdmcdqbridge.saas.dto.*
import com.catenax.bpdmcdqbridge.saas.entity.ImportEntry
import com.catenax.bpdmcdqbridge.saas.repository.ImportEntryRepository
...
class PartnerImportPageService(
    ...
    private fun addNewMetadata(partners: Collection<BusinessPartnerSaas>){
        partners
            ...
            .minus(poolClient.metadata().getIdentifierStati(PaginationRequest()).content.map { it.technicalKey }.toSet())
            .values
        ...
        partners
            ...
            .associateBy { it.technicalKey }
            .minus( poolClient.metadata().getIdentifierTypes(PaginationRequest()).content.map { it.technicalKey }.toSet())
            .values
            .map { mappingService.toRequest(it) }
    ...
    private fun createPartners(partners: Collection<BusinessPartnerSaas>):
        ...
        val createdLegalEntities = if (legalEntities.isNotEmpty()) poolClient.legalEntities().createBusinessPartners(legalEntities).entities else emptyList()
        val createdSites  = if (sites.isNotEmpty()) poolClient.sites().createSite(sites).entities else emptyList()
        val createdAddresses = if (addresses.isNotEmpty()) poolClient.addresses().createAddresses(addresses).entities else emptyList()
    ...
    private fun updatePartners(partners: Collection<BusinessPartnerWithBpn>):
        ...
        val createdLegalEntities = if (legalEntities.isNotEmpty()) poolClient.legalEntities().updateBusinessPartners(legalEntities).entities else emptyList()
        val createdSites = if (sites.isNotEmpty()) poolClient.sites().updateSite(sites).entities else emptyList()
        val createdAddresses = if (addresses.isNotEmpty()) poolClient.addresses().updateAddresses(addresses).entities else emptyList()
    ...
    private fun partitionBpnFound(partners: Collection<BusinessPartnerWithBpn>): Pair<Collection<BusinessPartnerWithBpn>, Collection<BusinessPartnerWithBpn>> {
        ...
        val foundBpnSs = poolClient.sites().searchSites(SiteBpnSearchRequest(sites =  bpnSs),
            PaginationRequest(size = if(!bpnSs.isEmpty()) bpnSs.size else 1))
            .content.map { it.site.bpn }

        val foundBpnAs = poolClient.addresses().searchAddresses(AddressPartnerBpnSearchRequest(addresses =  bpnAs),
