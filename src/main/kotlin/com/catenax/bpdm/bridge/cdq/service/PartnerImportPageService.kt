...
import com.catenax.bpdm.bridge.cdq.dto.*
...
class PartnerImportPageService(
    ...
    private fun createPartners(partners: Collection<BusinessPartnerSaas>):
        ...
        val createdLegalEntities = if (legalEntities.isNotEmpty()) poolClient.legalEntities()
            .createBusinessPartners(legalEntities).entities else emptyList()
        val createdSites = if (sites.isNotEmpty()) poolClient.sites().createSite(sites).entities else emptyList()
        val createdAddresses =
            if (addresses.isNotEmpty()) poolClient.addresses().createAddresses(addresses).entities else emptyList()

        val legalEntityImportEntries =
            createdLegalEntities.mapNotNull { if (it.index != null) ImportEntry(it.index!!, it.bpn) else null }
        val siteImportEntries =
            createdSites.mapNotNull { if (it.index != null) ImportEntry(it.index!!, it.bpn) else null }
        val addressImportEntries =
            createdAddresses.mapNotNull { if (it.index != null) ImportEntry(it.index!!, it.bpn) else null }
    ...
    private fun updatePartners(partners: Collection<BusinessPartnerWithBpn>):
        ...
        val createdLegalEntities = if (legalEntities.isNotEmpty()) poolClient.legalEntities()
            .updateBusinessPartners(legalEntities).entities else emptyList()
        val createdSites = if (sites.isNotEmpty()) poolClient.sites().updateSite(sites).entities else emptyList()
        val createdAddresses =
            if (addresses.isNotEmpty()) poolClient.addresses().updateAddresses(addresses).entities else emptyList()
    ...
    private fun determineParentId(children: Collection<BusinessPartnerSaas>): Collection<BusinessPartnerWithParentId> {
        return children.mapNotNull { child ->
            val parentId =
                child.relations.firstOrNull { id -> id.type?.technicalKey == adapterProperties.parentRelationType }?.startNode
            if (parentId != null) {
    ...
    private fun partitionHasNoBpnAndValidBpn(partners: Collection<BusinessPartnerSaas>): Pair<Collection<BusinessPartnerSaas>, Collection<BusinessPartnerWithBpn>> {
        ...
        val consequence =
            if (adapterProperties.treatInvalidBpnAsNew) "Record will be treated as having no BPN." else "Record will be ignored."
        bpnMissing.forEach {
        ...
        val hasNoBpn =
            if (adapterProperties.treatInvalidBpnAsNew) hasNoBpnIdentifier + bpnMissing.map { it.partner } else hasNoBpnIdentifier
    ...
    private fun partitionBpnFound(partners: Collection<BusinessPartnerWithBpn>): Pair<Collection<BusinessPartnerWithBpn>, Collection<BusinessPartnerWithBpn>> {
        ...
        val foundBpnLs = poolClient.legalEntities().searchSites(bpnLs = bpnLs).body!!.map { it.bpn }

        val foundBpnSs = poolClient.sites().searchSites(
            SiteBpnSearchRequest(sites = bpnSs),
            PaginationRequest(size = if (!bpnSs.isEmpty()) bpnSs.size else 1)
        )
            .content.map { it.site.bpn }

        val foundBpnAs = poolClient.addresses().searchAddresses(
            AddressPartnerBpnSearchRequest(addresses = bpnAs),
            PaginationRequest(size = if (!bpnSs.isEmpty()) bpnAs.size else 1)
        )
            .content.map { it.address.bpn }
