...
import com.catenax.bpdm.bridge.cdq.config.BpnConfigProperties
...
class SaasRequestMappingService(
    ...
    private val saasConfigProperties: SaasAdapterConfigProperties
) {
    fun toSaasModel(legalEntity: LegalEntityGateInputResponse): BusinessPartnerSaas {
        return toSaasModel(legalEntity.legalEntity, legalEntity.externalId, legalEntity.bpn)
    }

    fun toSaasModel(site: SiteGateInputResponse): BusinessPartnerSaas {
    ...
    fun toSaasModel(address: AddressGateInputResponse): BusinessPartnerSaas {
        return BusinessPartnerSaas(
    ...
    private fun toLegalFormSaas(technicalKey: String?) =
        if (technicalKey != null) LegalFormSaas(technicalKey = technicalKey) else null
    ...
    private fun toSaasModel(version: AddressVersionDto): AddressVersionSaas? {
        ...
        return if (languageSaas == null && characterSetSaas == null) null else AddressVersionSaas(
            languageSaas,
            characterSetSaas
        )
    }
    ...
    private fun toSaasModel(adminArea: AdministrativeAreaDto, languageCode: LanguageCode): AdministrativeAreaSaas =
        AdministrativeAreaSaas(
            adminArea.value,
            adminArea.shortName,
            toKeyNameUrlTypeSaas(adminArea.type),
    ...
    private fun toSaasModel(locality: LocalityDto, languageCode: LanguageCode): LocalitySaas =
        LocalitySaas(
            toKeyNameUrlTypeSaas(locality.type),
            locality.shortName,
            locality.value,
            toLanguageSaas(languageCode)
        )
    ...
    private fun toSaasModel(
        deliveryPoint: PostalDeliveryPointDto,
    ...
    private fun toSaasModel(premise: PremiseDto, languageCode: LanguageCode): PremiseSaas =
        PremiseSaas(
            toKeyNameUrlTypeSaas(premise.type),
            premise.shortName,
            premise.number,
            premise.value,
            toLanguageSaas(languageCode)
